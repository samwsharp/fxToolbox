<template>
    ...
    <view-trade-objectives @complete="data.canTakeTrade = $event"></view-trade-objectives>
    <div @mousedown="data.copyingToClipboard = true" @click="copyLotsToClipboard()" class="w-5/6 cursor-pointer rounded mx-auto my-5 p-5" :class="{'bg-green-200': data.canTakeTrade, 'bg-red-200': !data.canTakeTrade, 'bg-emerald-200': data.copyingToClipboard }">
        <div class="grid grid-cols-4">
            ...
            <template v-if="data.pips">
                ...
                <p class="mb-0 text-right"><strong>R:</strong> {{ $filters.toFixed(r) }} (${{ $filters.toFixed(fees+(maxRisk*3)) }})</p>
            </template>
        </div>
    </div>
    <h1 class="text-center mt-5 font-semibold text-white text-sm"><i>By Sam Sharp</i></h1>
    <vue-modal></vue-modal>
    <div @click="modal.open('edit-trade-objectives')" class="rounded-full text-white bg-sky-600 absolute p-2 bottom-5 right-5 cursor-pointer">
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16"> <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z"/> <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5v11z"/> </svg>
    </div>
    <div @click="modal.open('edit-risk-parameters')" class="rounded-full text-white bg-sky-600 absolute p-2 bottom-5 right-16 cursor-pointer">
...
export default {
    ...
    setup () {
        ...
        const maxRisk = computed(() => store.state.riskPreferences.maxRisk).value;

        // Calculate required R value to make up loss of any fees
        const r = computed(() => ((maxRisk*3) + fees.value) / risk.value);

        // Calculate the dollar risk per trade
        const risk = computed(() => {
            let amount = (maxRisk / (3.5 / data.pips / 0.5 + 1));
